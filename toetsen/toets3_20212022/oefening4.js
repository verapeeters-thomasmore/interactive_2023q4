...
function studentenVoorVak(studenten, vak){
    return studenten.filter(s => s[vak]).map(s=>s.naam);
}
...
function puntenVoorVak(studenten, vak){
    return studenten.filter(s => s[vak]).map(s=>({naam: s.naam, punten: s[vak]}));
}
...
function gemiddeldeVoorVak(studenten, vak){
    return studenten.filter(s => s[vak]).reduce((sum, s)=> sum+Number(s[vak]), 0)/studenten.filter(s => s[vak]).length;
}
